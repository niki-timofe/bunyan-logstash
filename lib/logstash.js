...
function LogstashStream(options, customFormatter) {
  options = options || {};
  ...
  this.customFormatter = customFormatter || function (rec) {
    var level = rec.level;
...
LogstashStream.prototype.write = function logstashWrite(entry) {
  ...
  var {msg, recf} = this.customFormatter(rec)

  this.send(JSON.stringify(extend({}, msg, recf), bunyan.safeCycles()));
};
